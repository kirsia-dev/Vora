...
local VoraLib
pcall(function()
    local src = game:HttpGet("https://raw.githubusercontent.com/juansyahrz17-prog/Coralx/refs/heads/main/lib.lua")
    if type(src) ~= "string" or #src == 0 then
        warn("[VoraHub] HttpGet gagal, type:", type(src))
        return
    end
    local fn, compileErr = loadstring(src)
    if not fn then
        warn("[VoraHub] Compile error:", compileErr)
        return
    end
    VoraLib = fn()
end)
if not VoraLib then
    warn("[VoraHub] Gagal load VoraLib!")
end
...
-- =====================================================
-- VORALIB COMPATIBILITY SHIM (ModernV2 API → VoraLib)
-- Translates existing section/tab API ke VoraLib calls
-- =====================================================
local _VoraWindow = nil
local _ConfigElements = {}

local function _makeVoraSectionProxy(voraTab)
    local proxy = {}

    function proxy:AddToggle(cfg)
        local el = { Value = cfg.Default or false, _cb = cfg.Callback }
        function el:Set(v)
            self.Value = v
            if self._cb then pcall(self._cb, v) end
        end
        pcall(function()
            voraTab:CreateToggle({
                Name     = cfg.Name or "",
        ...
        if cfg.Flag and cfg.Flag ~= "" then _ConfigElements[cfg.Flag] = el end
        return el
    end

    function proxy:AddSlider(cfg)
        local el = { Value = cfg.Default or cfg.Min or 0, _cb = cfg.Callback }
        function el:Set(v)
            self.Value = v
            if self._cb then pcall(self._cb, v) end
        end
        pcall(function()
            voraTab:CreateSlider({
                Name     = cfg.Name or "",
                Min      = cfg.Min or 0,
                Max      = cfg.Max or 100,
    ...
    function proxy:AddButton(cfg)
        pcall(function()
            voraTab:CreateButton({
                Name     = cfg.Name or "",
                SubText  = "",
    ...
    function proxy:AddDropdown(cfg)
        local el = { Value = cfg.Default, _cb = cfg.Callback }
        function el:Set(v)
            self.Value = v
            if self._cb then pcall(self._cb, v) end
        end
        pcall(function()
            voraTab:CreateDropdown({
                Name     = cfg.Name or "",
                Items    = cfg.Values or cfg.Items or {},
                Default  = cfg.Default,
                Callback = function(v)
                    el.Value = v
                    if cfg.Callback then pcall(cfg.Callback, v) end
    ...
    function proxy:AddColorPicker(cfg)
        -- VoraLib stub — tidak ada ColorPicker native
        local el = { Value = cfg.Default or Color3.fromRGB(255, 255, 255), _cb = cfg.Callback }
        function el:Set(v)
            self.Value = v
            if self._cb then pcall(self._cb, v) end
        end
    ...
    function proxy:AddInput(cfg)
        local el = { Value = cfg.Default or "", _cb = cfg.Callback }
        function el:Set(v)
            self.Value = v
            if self._cb then pcall(self._cb, v) end
        end
    ...
    -- AddTextInput → mapped ke CreateInput (numeric input field)
    function proxy:AddTextInput(cfg)
        local el = { Value = cfg.Default or "", _cb = cfg.Callback }
        function el:Set(v)
            self.Value = v
            if self._cb then pcall(self._cb, v) end
    ...
    -- AddDivider & AddSection sebagai visual separator
    function proxy:AddDivider(cfg)
        if cfg and cfg.Text then
            pcall(function() voraTab:CreateSection({ Name = cfg.Text }) end)
        end
    end

    function proxy:AddSection(cfg)
        if cfg and cfg.Name then
            pcall(function() voraTab:CreateSection({ Name = cfg.Name }) end)
        end
        return _makeVoraSectionProxy(voraTab)
    end

    return proxy
end

local function _makeVoraTabProxy(voraTab)
    local proxy = _makeVoraSectionProxy(voraTab)

    -- AddCenterTabbox: buat sub-tab VoraLib terpisah per fitur grup
    function proxy:AddCenterTabbox(name)
        local tabboxProxy = {}
        function tabboxProxy:AddTab(cfg)
            local subTab
            pcall(function()
                subTab = _VoraWindow:CreateTab({
                    Name = cfg.Name or "",
                    Icon = cfg.Icon or ""
                })
            end)
            return _makeVoraSectionProxy(subTab or voraTab)
        end
        return tabboxProxy
    end
...
if VoraLib then
    pcall(function()
        _VoraWindow = VoraLib:CreateWindow({
            Name  = "VoraHub",
            Intro = true
        })
    end)
    ...
    if _VoraWindow then
        ...
        function Window:AddTab(cfg)
            local voraTab
            pcall(function()
                voraTab = _VoraWindow:CreateTab({
                    Name = cfg.Name or "",
                    Icon = cfg.Icon or ""
                })
            end)
            return _makeVoraTabProxy(voraTab or {
        ...
        -- Stub methods (tidak didukung VoraLib)
        function Window:SetAccount(cfg) end
        function Window:CreateHomeTab(cfg) end
        function Window:AttachMenuIcon(icon) end

        -- Notify awal
        pcall(function()
            _VoraWindow:Notify({
                Title    = "VoraHub",
                Content  = "Violence District v1.4.2 Loaded!",
                Duration = 4
            })
        end)
    end
...
getgenv().VD = getgenv().VD or {
    ...
    AUTO_ToFAim           = false,
    AUTO_ToFAimRange      = 90,
    AUTO_ToFDotThreshold  = 0.5,   -- Minimum dot product kamera vs arah killer (cegah self-damage)
    AUTO_ToFDebug         = false,
    AUTO_ToFTracer        = false,  -- Tracer garis ke killer saat dalam range ToF
...
local ConfigFolderName = "VoraHub_VD"
local HttpService = game:GetService("HttpService")
...
-- Dynamic Remote Cache for stability & lobby compatibility
...
-- Cache remote ToF untuk auto aim
...
-- Flag: mencegah infinite recursion saat task.defer fire ToF
-- (deferred call bisa kembali ke hookmetamethod, flag ini memblok loop itu)
...
-- =====================================================
-- Fix self-damage:
--   1. Aim ke TORSO/CHEST (HRP + Y+1.5) bukan root bawah
--   2. Physics-accurate prediction: travelTime = dist / bulletSpeed (200 studs/s dari spy)
--   3. Dot product validation: skip jika kamera belum hadap killer (cegah self-damage)
                if _tofDeferred then
-- ini panggilan dari task.defer kita, langsung ke oldNamecall
                ...
                if bestRoot then
-- FIX 1: Origin tembakan = posisi GUN (args[1].Position)
-- Server pakai gun part sebagai titik awal bullet, BUKAN HRP survivor
-- Spy: FireServer(Character["Twist of Fate"]["Right Arm"].gun, dirVector)
                local gunPart = args[1]
                local gunPos
                pcall(function() gunPos = gunPart.Position end)
                gunPos = gunPos or myRoot.Position  -- fallback kalau gun nil

-- FIX 2: Target ke HumanoidRootPart (bestRoot) langsung
-- Jangan gunakan Torso karena Torso bisa wobble/bergetar akibat Ragdoll.lua atau animasi
                local targetCenter = bestRoot.Position

-- FIX 3: Physics-accurate bullet travel time
-- Dari spy VisualizeBullet: arg ke-3 = 200 (bullet speed studs/s)
                local BULLET_SPEED = 200
                local dist         = (targetCenter - gunPos).Magnitude
                local travelTime   = dist / BULLET_SPEED

-- Prediksi posisi killer saat bullet tiba
-- PENTING: Hilangkan velocity Y agar tembakan tidak melenceng ke tanah/langit saat killer di tanjakan
...
-- Arah dari GUN ke predicted position
                local dir    = targetPos - gunPos
                local newDir = (dir.Magnitude > 0.01) and dir.Unit or args[2]

-- Guard: Dot product validation cegah self-damage
-- Cukup dot product saja, raycast terlalu agresif kena spring LookScript
                ...
                if dotCheck < dotThreshold then
                        return  -- kamera belum hadap killer, skip
                                end

-- Fire dengan arah yang sudah dikoreksi
                                _tofDeferred = true
                                task.defer(function()
                                    pcall(function()
...
-- Kontrol ditambahkan ke tab Visual lewat getgenv().VORA_AddVisualESPControls.
-- =====================================================
do
    ...
    local function VORA_GetESPFolder()
        ...
        local old = parent:FindFirstChild("VoraHub_VisualESP") or parent:FindFirstChild("ZiaanHub_ESP")
...
if Window then
    ...
    AimFeatureTabs = addCenterFeatureTabbox(Tabs.Aim, "Aim Features", {
        ...
        { Key = "Spear", Name = "Spear", Icon = "solar:sword-bold" },
        { Key = "AutoAim", Name = "Auto Aim", Icon = "solar:magic-stick-3-bold" },
    })
    ...
    PlayerMiscFeatureTabs = addCenterFeatureTabbox(Tabs.Player, "Player Misc", {
        ...
        { Key = "Avatar", Name = "Copy Avatar", Icon = "solar:users-group-rounded-bold" },
    })
...
do -- Aim Tab
    ...
    spearSection:AddToggle({ Default = false, Name = "Silent Aim Spear (Veil)", Locked = getgenv().VoraHubTier ~= "Premium", TextLocked = "Premium Required", Flag = "Silent Aim Spear (Veil)", Callback = function(v) 
        if v and getgenv().VoraHubTier ~= "Premium" then
            pcall(VD_Notify, "Premium Required ✨", "Fitur Silent Aim Spear (Veil) hanya untuk pengguna Key Premium!", 5)
            return
        end
    ...
    tofAimSection:AddToggle({ Default = false, Name = "Auto Aim Twist of Fate", Locked = getgenv().VoraHubTier ~= "Premium", TextLocked = "Premium Required", Flag = "Auto Aim Twist of Fate", Callback = function(v) 
        if v and getgenv().VoraHubTier ~= "Premium" then
            pcall(VD_Notify, "Premium Required ✨", "Fitur Auto Aim Twist of Fate hanya untuk pengguna Key Premium!", 5)
            return
        end
        VD.AUTO_ToFAim = v 
    end })
    tofAimSection:AddToggle({ Default = false, Name = "ToF Tracer", Locked = getgenv().VoraHubTier ~= "Premium", TextLocked = "Premium Required", Flag = "ToF Tracer", Callback = function(v) 
        if v and getgenv().VoraHubTier ~= "Premium" then
            pcall(VD_Notify, "Premium Required ✨", "Fitur ToF Tracer hanya untuk pengguna Key Premium!", 5)
            return
        end
        VD.AUTO_ToFTracer = v 
    end })
    ...
    tofAimSection:AddSlider({
        -- Aim Strictness: dot product minimum kamera vs arah ke killer
        -- 0.0 = fire apapun arahnya (berisiko self-damage)
        -- 0.5 = harus dalam 60Â° dari killer (aman, default referensi = 2.1 â†’ ~0.5 dot)
        -- 1.0 = harus tepat lurus ke killer
...
do -- Survivor Tab
    ...
    combatSurv:AddToggle({ Default = false, Name = "Auto Heal All", Locked = getgenv().VoraHubTier ~= "Premium", TextLocked = "Premium Required", Flag = "Auto Heal All", Callback = function(v) 
        if v and getgenv().VoraHubTier ~= "Premium" then
            pcall(VD_Notify, "Premium Required ✨", "Fitur Auto Heal All hanya untuk pengguna Key Premium!", 5)
            return
        end
        setAutoHealAll(v) 
    end })
...
do -- Killer Tab
    ...
    combatKiller:AddToggle({ Default = false, Name = "Inf Grab (Myers)", Locked = getgenv().VoraHubTier ~= "Premium", TextLocked = "Premium Required", Flag = "Inf Grab (Myers)", Callback = function(v) 
        if v and getgenv().VoraHubTier ~= "Premium" then
            pcall(VD_Notify, "Premium Required ✨", "Fitur Inf Grab (Myers) hanya untuk pengguna Key Premium!", 5)
            return
        end
        setMyersGrab(v) 
    end })
    combatKiller:AddToggle({ Default = false, Name = "Undraggable Button (Inf Grab)", Locked = getgenv().VoraHubTier ~= "Premium", TextLocked = "Premium Required", Flag = "Undraggable Button (Inf Grab)", Callback = function(v) 
        if v and getgenv().VoraHubTier ~= "Premium" then
            pcall(VD_Notify, "Premium Required ✨", "Fitur Undraggable Button (Inf Grab) hanya untuk pengguna Key Premium!", 5)
            return
        end
...
local function VD_UpdateSurvivorWarnings()
    ...
    local now = tick()
    if VD._WarnKillerNext and now < VD._WarnKillerNext then return end
    VD._WarnKillerNext = now + 0.15
...
local function VD_UpdateBypassGate()
    ...
    if VD._NextBypassGate and tick() < VD._NextBypassGate then return end
    VD._NextBypassGate = tick() + 1
...
local function CheckMapChange()
    ...
    if CurrentMapName ~= mapName then
        -- print("[VORA HUB] Map berubah: " .. tostring(CurrentMapName) .. " -> " .. mapName)
...
-- HIJAU = aman (dot >= threshold) â†’ akan kena Torso killer
-- MERAH = berbahaya (dot < threshold) â†’ akan self-damage
-- =====================================================
do
    local TweenService = game:GetService("TweenService")
    ...
    -- Helper: gambar garis Aâ†’B di layar
    ...
    local COLOR_SAFE   = Color3.fromRGB(0, 220, 100)   -- hijau
    local COLOR_DANGER = Color3.fromRGB(255, 55, 55)    -- merah
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if tick() - _lastTracerScan < 0.05 then return end  -- ~20fps
        ...
        -- =============================================
        -- PREDIKSI WARNA: dot product & raycast self-damage
        -- Logika sama persis dengan ToF auto aim intercept
        -- =============================================
        -- Titik asal: dari Right Arm / tangan kanan (tempat Gun berada) atau Root
        ...
        -- Update warna hanya kalau berubah (cegah thrash tween)
        if isSafe ~= _lastSafe then
            local targetColor = isSafe and COLOR_SAFE or COLOR_DANGER
            -- Tween smooth color transition 0.1s
        ...
        -- Hitung persentase aim (0â€“100%) untuk ditampilkan di label
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        -- Scan pallet dari VORA_Cache + langsung dari workspace
        ...
        -- Iterasi VORA_Cache.Pallets untuk cari Palletwrong terdekat
...
-- Logika aim sudah diintegrasikan ke hookmetamethod di setupAntiFail
-- untuk menghindari konflik double __namecall hook
-- =====================================================
    ...
    if DrawingAvailable then
        if VD.DRAWING_ESP then
            if canUpdateESP then
                ...
                -- Object ESP sudah dihapus. Drawing ESP PC tetap hanya render player.
